...
class UpdateMemberTotalsJob implements ShouldQueue
{
    ...
    {
        // Prevent overlapping runs (e.g., if last night ran long)
        $lock = Cache::lock('update_member_totals_lock', 55 * 60); // 55 minutes
        if (! $lock->get()) {
            Log::warning('UpdateMemberTotalsJob: another run is in progress, skipping.');
            return;
        }

        try {
            // 1) Reset totals to zero to avoid stale values
            DB::statement("
            ...
            Log::info('UpdateMemberTotalsJob: totals updated (shares/fosa/capital).');
        } finally {
            $lock->release();
        }
    }
