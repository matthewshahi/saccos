...
class MemberReportController extends Controller
{
    ...
    public function fetchReportData(Request $request)
    {
        ...
        // Fetch members with search filters
        $membersQuery = DB::table('sacco_members')
            ...
            ->orderBy('member_name', 'asc') // Ensure alphabetical order
            ->offset($offset)
            ->limit($limit);
        ...
        $totals = [
            ...
            'total_fosa' => 0,
            'total_loans' => 0,
            'loan_type_totals' => [], // Totals for each loan type
        ];
        ...
        foreach ($members as $member) {
            ...
            // Calculate outstanding loans for each type
        ...
        return response()->json([
            ...
            'data' => $reportData,
            'totals' => $totals,
            'hasMoreData' => count($members) === $limit, // Check if there are more records
        ]);
    }
}