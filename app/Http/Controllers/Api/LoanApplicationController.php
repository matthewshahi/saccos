...
class LoanApplicationController extends Controller
{
    ...
    */
        $loanTypes = DB::table('sacco_loan_types')
            ...
            ->orderByRaw('
        CASE
            WHEN loan_type_auto_approval = 1
             AND loan_type_instant_disbursement = 1
            THEN 0
            ELSE 1
        END ASC
    ')
            ...
            ->map(function ($t) use ($memberRow, $monthsInSacco) {
                ...
                $autoApproval =
                    (int) ($t->loan_type_auto_approval ?? 0) === 1;

                $instantDisbursement =
                    (int) ($t->loan_type_instant_disbursement ?? 0) === 1;

                $isInstantMoney =
                    $autoApproval && $instantDisbursement;
                ...
                return [
                    ...
                    (int) $t->loan_type_qualification_period,

                    'instant_qualification' =>
                    $this->isInstantLoanType($t),

                    'auto_approval' => $autoApproval,
                    'instant_disbursement' => $instantDisbursement,
                    'instant_money' => $isInstantMoney,

                    'insurable' =>
                    strtoupper(
                        (string) ($t->loan_type_insurable ?? 'N')
                    ) === 'Y',

                    'share_factor' =>
                    (int) ($t->loan_type_share_factor ?? 0),

                    'loan_type_guaranteable_percent' =>
                    (float) (
                        $t->loan_type_guaranteable_percent ?? 0
                    ),
                    ...
                    'qualified_amount' =>
                    $qualification['qualified_amount'],
                    ...
                    $qualification['qualification_shortfall'],

                    'months_in_sacco' => $monthsInSacco,

                    'current_total_loans' =>
                    $qualification['current_total_loans'],
                ];
            })
            ->values();
