...
class LedgerRebuildController extends Controller
{
    public function rebuild(Request $request)
    {
        ...
        try {
            ...
            // ✅ Step 1: Check required defaults
            $requiredDefaults = [
            ...
            foreach ($requiredDefaults as $key) {
                $val = DB::table('sacco_defaults')->where('default_name', $key)->value('default_value');
                if (empty($val) || $val == 0) {
                    $missing[] = $key;
                }
            }
            ...
            if ($missing) {
                $msg = "❌ Ledger rebuild aborted. Missing/unset defaults: " . implode(', ', $missing);
                Log::error($msg);
            ...
            DB::table('sacco_sub_account')->update(['sub_account_debit' => 0, 'sub_account_credit' => 0]);
            DB::table('sacco_main_account')->update(['main_account_debit' => 0, 'main_account_credit' => 0]);

            // ✅ Step 3: Map transaction tables
            $mappings = [
                [
                    'table' => 'sacco_shares',
            ...
            foreach ($mappings as $map) {
                $records = DB::table($map['table'])->get();

                foreach ($records as $rec) {
                    $docNo = trim($rec->{$map['doc_col']} ?? '');
                    $amount = (float) $rec->{$map['amount_col']};
                    $desc = trim($rec->{$map['desc_col']} ?? '');
                    ...
                    // Detect Paybill vs Bank
                    $isMpesa = stripos($docNo, 'Paybill') !== false;
                    $debitKey = $isMpesa ? 'default_mpesa_in_account' : 'default_bank_account';
                    ...
                    if (empty($debitAcc) || empty($creditAcc)) {
                        Log::warning("SKIPPED {$docNo}: missing subaccount ({$debitKey}={$debitAcc}, {$creditKey}={$creditAcc})");
                        continue;
                    }

                    if ($desc === '') $desc = "{$map['table']} transaction - {$docNo}";
