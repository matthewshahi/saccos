...
class PublicLoansController extends Controller
{
    ...
    // ✅ Fixed (Flat) interest: constant interest each month
    private function calculateFixedEMI($principal, $rate, $months)
    {
        // convert annual rate to monthly rate
        $monthlyRate = $rate / 12 / 100;

        // total interest across loan period
    ...
    private function generateFixedSchedule($principal, $rate, $months, $emi)
    {
        ...
        // monthly interest constant for flat rate
        $monthlyInterest = $principal * $monthlyRate;

        // principal portion is equally distributed
        $principalPayment = $principal / $months;

        for ($i = 1; $i <= $months; $i++) {
            $balance = $principal - ($principalPayment * $i);
