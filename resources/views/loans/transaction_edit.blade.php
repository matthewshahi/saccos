        ...
        <div class="card mb-3">
            <div class="card-body d-flex justify-content-between align-items-center flex-wrap gap-2 py-3">
                <div>
                    ...
                    <small class="text-muted">Edit a member loan transaction in batch <strong>{{ $batch->batch_reference }}</strong>.</small>
                </div>
                            ...
                            <div class="row">
                                <div class="col-md-12 mb-3 position-relative">
                                    ...
                                    <input
                                        ...
                                        autocomplete="off"
                                        required
                                        data-member-id="{{ $transaction->batch_trans_member_id }}"
                                        value="{{ old('batch_trans_member_id', $transaction->member_name . ' - (' . $transaction->member_sacco_id . ')') }}"
                                    >
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="batch_trans_loan_type" id="loan_type" class="form-control @error('batch_trans_loan_type') is-invalid @enderror" required>
                                        ...
                                        @foreach($loanTypes as $type)
                                            <option value="{{ $type->loan_type_id }}" {{ old('batch_trans_loan_type', $transaction->batch_trans_loan_type) == $type->loan_type_id ? 'selected' : '' }}>
                                                {{ $type->loan_type_name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="batch_trans_loan_category" id="loan_category" class="form-control @error('batch_trans_loan_category') is-invalid @enderror" required>
                                        ...
                                        @foreach($loanCategories as $category)
                                            <option value="{{ $category->loan_category_id }}" {{ old('batch_trans_loan_category', $transaction->batch_trans_loan_category) == $category->loan_category_id ? 'selected' : '' }}>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input
                                        ...
                                        required
                                        value="{{ old('batch_trans_loan_amount', $transaction->batch_trans_loan_amount) }}"
                                    >
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input
                                        ...
                                        required
                                        value="{{ old('batch_trans_loan_duration', $transaction->batch_trans_loan_duration) }}"
                                    >
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input
                                        ...
                                        value="{{ old('batch_trans_commission_amount', $transaction->batch_trans_commission) }}"
                                    >
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input
                                        ...
                                        required
                                        value="{{ old('batch_trans_doc_no', $transaction->batch_trans_doc_no) }}"
                                    >
                                ...
                                <div class="col-md-12 mb-3">
                                    ...
                                    <textarea
                                        ...
                                        rows="3"
                                    >{{ old('batch_trans_description', $transaction->batch_trans_description) }}</textarea>
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-bordered align-middle" id="other-charges-table">
                                    ...
                                    <tbody>
                                        @php
                                            $displayCharges = old('other_charges');

                                            if ($displayCharges === null) {
                                                $displayCharges = [];

                                                foreach (($existingOtherCharges ?? []) as $charge) {
                                                    $displayCharges[] = [
                                                        'deduction_type_id' => $charge->batch_trans_deduction_deduction_type_id,
                                                        'effect' => $charge->batch_trans_deduction_effect,
                                                        'value_type' => $charge->batch_trans_deduction_value_type,
                                                        'amount' => $charge->batch_trans_deduction_amount,
                                                        'description' => $charge->batch_trans_deduction_description,
                                                    ];
                                                }
                                            }

                                            if (count($displayCharges) === 0) {
                                                $displayCharges = [[]];
                                            }
                                        @endphp

                                        @foreach($displayCharges as $i => $charge)
                                            <tr class="charge-row">
                                                <td>
                                                    <select
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-bordered align-middle" id="guarantors-table">
                                    ...
                                    <tbody>
                                        @php
                                            $displayGuarantors = old('guarantors');

                                            if ($displayGuarantors === null) {
                                                $displayGuarantors = [];

                                                foreach (($guarantors ?? []) as $g) {
                                                    $displayGuarantors[] = [
                                                        'member' => $g->member_name . ' - (' . $g->member_sacco_id . ')',
                                                        'amount' => $g->guarantors_amount_guaranteed,
                                                        'free_shares' => '',
                                                        'member_id' => $g->guarantor_member_id,
                                                    ];
                                                }
                                            }

                                            if (count($displayGuarantors) === 0) {
                                                $displayGuarantors = [[]];
                                            }
                                        @endphp

                                        @foreach($displayGuarantors as $i => $guarantor)
                                            <tr>
                                                <td class="position-relative">
                                                    <input
                                                        ...
                                                        autocomplete="off"
                                                        data-member-id="{{ $guarantor['member_id'] ?? '' }}"
                                                        value="{{ $guarantor['member'] ?? '' }}"
                                                    >
...
function showHintMembers(str) {
    ...
    function selectMember(label, memberId) {
        ...
        memberInput.setAttribute('data-member-id', memberId);
        document.getElementById('suggestions-box').innerHTML = '';
...
function showHintGuarantor(str, index) {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.querySelectorAll('.guarantor-member').forEach(function(input, index) {
