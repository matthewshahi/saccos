...
<head>
  ...
  <title>Shares Receipt - {{ $rec->share_doc_no }}</title>
  <style>
...
<body>
  ...
  <div class="receipt-title">Receipt #SAV-{{ $rec->share_id }}</div>

  <!-- Member & Transaction Info -->
  <table class="info">
    <tr>
      <td><strong>Document No:</strong> {{ $rec->share_doc_no }}</td>
      <td><strong>Date:</strong> {{ \Carbon\Carbon::parse($rec->share_date_paid)->format('d M Y H:i') }}</td>
    </tr>
    <tr>
    ...
    <tr>
      <td><strong>Period:</strong> {{ $rec->share_period ?? '-' }}</td>
      <td><strong>Transaction Date:</strong> {{ \Carbon\Carbon::parse($rec->share_transdate)->format('d M Y H:i') }}</td>
    </tr>
  </table>
  ...
  <table class="details">
    ...
    <tr>
      ...
      <td class="amount">
        @if($rec->share_amount_paying >= 0)
          <span class="text-success">+{{ number_format($rec->share_amount_paying, 2) }}</span>
        @else
          <span class="text-danger">{{ number_format(abs($rec->share_amount_paying), 2) }}</span>
        @endif
      </td>
    </tr>
  </table>
  ...
  <table class="info">
    <tr>
      <td><strong>Paid By:</strong> {{ $rec->share_paid_by }}</td>
      <td><strong>Entered By:</strong> {{ $rec->entered_by_name ?? 'System' }}</td>
    </tr>
    <tr>
      <td colspan="2"><strong>IP Address:</strong> {{ $rec->share_ip }}</td>
    </tr>
  </table>
  ...
  <div class="footer">
  Thank you for saving with {{ $companyName }}.<br>
